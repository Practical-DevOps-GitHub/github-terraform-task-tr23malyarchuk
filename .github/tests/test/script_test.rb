...
class ScriptTest < Test::Unit::TestCase
  ...
  def test_develop_present
    ...
    refute(actual, 'Branch develop should not be present')
  end
  ...
  def test_develop_default
    ...
    expected = 'main'
    assert_equal(expected, actual, 'Default branch isn\'t main')
  end

  def test_codeowners_contains_user
    user_name = 'softservedata'
    content = @obj.file_branch('CODEOWNERS', 'main')
    assert_nil(content, 'File CODEOWNERS exists on main branch')
  end
  ...
  def test_deny_merge_main
    ...
    assert_nil(actual, 'We should not allow merge to main branch without PR reviews')
  end  
  
  def test_deny_merge_develop
    actual = @obj.rules_required_pull_request_reviews('develop')
    assert_nil(actual, 'We should not allow merge to develop branch without PR reviews')
  end  
